...
// LOGIN / LOGOUT ADMIN — lewat fungsi admin_login() (butuh kunci rahasia + role admin)
document.getElementById('formAdminLogin')?.addEventListener('submit', async (e) => {
    ...
    const { data, error } = await supabase.rpc('admin_login', { p_admin_secret: ADMIN_RPC_SECRET, p_no_wa: noWa, p_password: pass });
    ...
    localStorage.setItem('sesiAdmin', JSON.stringify(data[0]));
...
window.loadPesananData = async function () {
    ...
    table.innerHTML = filteredData.map(item => `
        <tr>
            ...
            <td><a href="https://wa.me/${item.no_wa}" target="_blank" style="color:#ef4444; text-decoration:none;">${item.no_wa}</a></td>
...
async function loadSellerReqData() {
    ...
    const { data } = await supabase
        .from('users')
        .select('id, no_wa, nama, asal, umur, nomor_dana, role, seller_status, seller_verified, created_at')
    ...
    pendingTable.innerHTML = pending.length > 0 ? pending.map(u => `
        <tr>
            <td><b>${u.nama}</b></td>
            <td><a href="https://wa.me/${u.no_wa}" target="_blank" style="color:#ef4444; text-decoration:none;">${u.no_wa}</a></td>
            <td>${u.asal || '-'}</td>
            <td>${u.umur || '-'}</td>
            <td>${u.nomor_dana || '-'}</td>
    ...
    `).join('') : `<tr><td colspan="7" style="text-align:center; color:#64748b;">Tidak ada pengajuan baru.</td></tr>`;
...
window.setujuiSeller = async function (id) {
    const { error } = await supabase.rpc('admin_setujui_seller', { p_admin_secret: ADMIN_RPC_SECRET, p_user_id: id });
...
window.tolakSeller = async function (id) {
    const { error } = await supabase.rpc('admin_tolak_seller', { p_admin_secret: ADMIN_RPC_SECRET, p_user_id: id });
    if (error) { Swal.fire({ icon: 'error', title: 'Gagal', text: error.message }); return; }
...
window.toggleVerifiedSeller = async function (id, statusSaatIni) {
    await supabase.rpc('admin_toggle_verified_seller', { p_admin_secret: ADMIN_RPC_SECRET, p_user_id: id, p_status_baru: !statusSaatIni });
    loadSellerReqData();
};
...
window.cabutSeller = async function (id) {
    ...
    await supabase.rpc('admin_cabut_seller', { p_admin_secret: ADMIN_RPC_SECRET, p_user_id: id });
    loadSellerReqData();
};
    ...
    if (data && data.length > 0) {
        list.innerHTML = data.map(info => `
            <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); padding: 12px; border-radius: 10px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: flex-start;">
                <div>
                    <div style="font-weight: 700; font-size: 14px; color: #ef4444;">${info.judul}</div>
